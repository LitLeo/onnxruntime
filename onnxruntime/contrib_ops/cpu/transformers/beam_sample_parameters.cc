...
#include "contrib_ops/cpu/transformers/beam_sample_parameters.h"
...
Status BeamSampleParameters::Validate() const {
...
void BeamSampleParameters::ParseFromAttributes(const OpKernelInfo& info) {
  ...
  //min_tokens_to_keep = static_cast<int>(info.GetAttrOrDefault<int64_t>("min_tokens_to_keep", 2));
}

void BeamSampleParameters::ParseFromInputs(OpKernelContext* context) {
  ...
  auto* top_k_tensor = context->Input<Tensor>(7);
  top_k = top_k_tensor ? static_cast<int>(*top_k_tensor->Data<int32_t>()) : 1;
  ORT_ENFORCE(top_k > 0,
              "top_k shall be > 0 integer, got ", top_k);
}

void BeamSampleParameters::SetSubgraphParameters(int vocabulary_size, int heads, int hidden_size_per_head, int layers) {
